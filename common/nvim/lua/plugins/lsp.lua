return {
    {
        ...
        config = function()
            ...
            local yamlls_cmd = find_lsp_executable('yaml-language-server', {
                ...
                vim.fn.expand('~/.npm-global/bin/yaml-language-server'),
                vim.fn.expand('~/.local/share/npm/bin/yaml-language-server'),
                '/nix/profile/bin/yaml-language-server',
            })
            vim.lsp.config('yamlls', {
                cmd = { yamlls_cmd, '--stdio' },
                filetypes = { 'yaml', 'yaml.dockercompose', 'yml' },
                root_markers = { 
                    { '.yamllint.yml', '.yamllint.yaml', 'yamllint.config.yml', 'yamllint.config.yaml' },
                    { 'package.json', 'docker-compose.yml', 'docker-compose.yaml', 'docker-compose.override.yml', 'docker-compose.override.yaml' },
                    { '.gitlab-ci.yml', '.github/workflows', 'azure-pipelines.yml', 'bitbucket-pipelines.yml' },
                    '.git'
                },
                settings = {
                    yaml = {
                        ...
                        schemas = {
                            ...
                            ['https://json.schemastore.org/azure-pipelines'] = 'azure-pipelines.yml',
            ...
            require("mason-lspconfig").setup {
                ensure_installed = { "pyright", "vtsls", "nil", "lua-language-server", "yaml-language-server" },
